# Excel Report Designer

## Overview
This module is used to  create reports for models and can be select the fields the user want to print.

## Installation

### Prerequisites
**Required Odoo Modules:**
- sale
- purchase
- account

### Installation Steps
1. Ensure all prerequisites are installed
2. Copy the `excel_report_designer` folder to your Odoo addons directory
3. Restart Odoo server
4. Update apps list: Settings → Apps → Update Apps List
5. Search for "Excel Report Designer"
6. Click Install

## Compatibility

### Odoo Version
- **Supported:** Odoo 18.0
- **Version:** 18.0.1.0.0

### Potential Conflicts
✅ No known conflicts detected

## Production Status
🎯 **PRODUCTION READY** - Safe for production deployment

## Module Information
- **Category:** Business
- **Author:** CBMS TECHNOLOGIES LTD
- **Maintainer:** CBMS TECHNOLOGIES LTD

## Support
For technical support or installation issues, please contact your system administrator or CBMS support team.

---
*This README was automatically generated for production deployment.*
