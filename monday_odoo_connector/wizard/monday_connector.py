    ...
    def get_boards(self, url, headers):
        ...
        for rec in response.json()['data']['boards']:
            ...
            for item in rec['groups']:
                existing_group = self.env['monday.group'].search([
                    ('group', '=', item['id']),
                ...
                if not existing_group:
                    board_obj.write({
                        'group_ids': [(0, 0, {
                            'name': item['title'], 'group': item['id']
                        }),]
                    })
            for item in rec['items_page']['items']:
                existing_items = self.env['monday.item'].search([
                    ('name', '=', item['name']),
                    ('board_id', '=', board_obj.id)
                ], limit=1)
                if not  existing_items:
                    board_obj.write({        # Update Items
                        'item_ids': [(0, 0, {
                            'name': item['name'],
                ...
                if rec['name'] == 'Contacts' and self.import_contact:
                    vals = {value['id']: value['text'] for value in
                            item['column_values']}
