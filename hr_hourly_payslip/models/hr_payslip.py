...
class HrPayslip(models.Model):
    ...
    def _onchange_employee_id(self):
        ...
        if self.employee_id.hourly_payslip:
    ...
    def _compute_total_hours(self):
        ...
        for record in self:
            ...
            ]).filtered( lambda ids: record.date_from <= ids.check_in.date() <= record.date_to
            ).mapped('worked_hours'))
    ...
    def _compute_hour_based_salary(self):
        ...
        employee. """
        for record in self:
            record.hour_based_salary = self.env['hr.employee'].browse(
                record.employee_id.id).hourly_cost * record.total_hours
